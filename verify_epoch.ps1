# ============================================================
# verify_epoch.ps1
# Verifies every proof in .\epoch_0002\proofs.json against onchain root
# ============================================================
...
$proofs = Get-Content ".\epoch_0002\proofs.json" -Raw | ConvertFrom-Json
...
foreach ($p in $proofs) {
    ...
    if ($ok) {
        Write-Host "[PASS] $($p.path)"
        $pass++
    } else {
        Write-Host "[FAIL] $($p.path)"
        $fail++
    }
}
